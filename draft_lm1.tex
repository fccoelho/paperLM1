...
observed data, are frequently overlooked. If the model is about a disease 
invading a new population one can simply assume the vast majority of the 
population to be susceptible. However for a disease which is well in its way to 
...
In the particular case of Dengue fever in Brasil, with four circulating 
...
in the late 1980s. For SIR-like models, the relevant initial values are the 
number of infectious and susceptible  individuals. The number of infectious 
...
continuously. The real challenge is to determine the number of susceptibles to 
...
any epidemic, which nevertheless acquire immunity. Lastly Demography and 
migrations affect the number of susceptible in ways which are not easy to 
determine.

The initial number of susceptibles -- $S_0$ -- is a a very powerful parameter 
with the regards to epidemic risk. Its value affects directly the local $R_0$, 
thus being intrumental in predicting epidemics, but it also affects the speed 
and size of the epidemic.

In this paper, we estimate the initial number of susceptibles right before 
every epidemic in the last 18 years. For this we use a simplified model of 
dengue transmission in which the vector is left out. We also model the mix of 
circulating serotypes without discriminating between them. We then fit the 
model output to observed data to estimate the $S_0$ before every 
recorded epidemic in the Brazilian city of Rio de Janeiro. Rio de Janeiro has 
bee through many Dengue epidemics of various serotypes and sizes.

\section*{Methods}
\subsection*{The data}
The Data used to fit the model consists of weekly incidence from 1996 to 2014 
in the city of Rio de Janeiro, Brazil. From this data, we calculated series 
effective reproduction number, $R_t$, time series using the method described by 
Nishiura et al.\cite{nishiura}.\todo[inline]{Add here more information about 
the calculation of Rt such as generation time used, moving windows, etc}
\subsection*{The model}
\subsection*{Parameter estimation using Bayesian melding}
