...
cases divided byt the population at risk.
For Dengue outbreaks and epidemics 
...
circulating virus(es).
To estimate the susceptibles we have we have fixed the 
tramissibility based on the $R_t$ series derived from data~\cite{nishiura}. 
Then, 
...
outbreaks from case data.\todo[inline]{anyone else?}
In the case of Dengue the 
...
In this section we will start by describing a method to estimate the 
effective 
reproductive number, $R_t$, and obtain its 
...
We employ these methods to analyze Dengue weekly incidence time series for the 
city of Rio de Janeiro from 1996 to 2014.
...
Also, one can chose $a_0$ and $b_0$ so as elicit meaningful prior distributions
for $R_t$.
...
But, as we can see in figure xx\todo{add 
figure with the four maps}, the disease spreads pretty much homogeneously over 
