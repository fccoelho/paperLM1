...
\section*{Abstract}
A method for estimation of the Attack ratio of an epidemic and the initial 
fraction of susceptibles, $S_0$, is proposed. The estimation methods obtain the 
Bayesian posterior distributions of the parameters by fitting a 
single-strain SIR transmission model with time-varying force of infection to 
the aggregated incidence data.  The data consists of 18 years of dengue 
incidence in the city of Rio de Janeiro, Brazil. An analytical method for 
deriving the probability distribution of the effective reproductive number, \rr.
...
For Dengue outbreaks and epidemics, it can be difficult to calculate the AR due 
to the lack of knowledge of the population at risk. The population at risk 
in the case of dengue, is the number of susceptibles to the circulating virus 
...
Without regular virological surveys, it is virtually impossible to determine 
...
with the force of infection. Thus the incorporation of the effective 
reproduction number, \rr, as a function of time, is crucial to an accurate 
estimation of the AR.

There are other sources of uncertainty impinging on the estimation of the AR
of a dengue epidemic. One of them is that there are a large number of 
asymptomatic cases in any epidemic, which nevertheless acquire immunity.
It is estimated that for every case reported, 10-20 are not seen by health 
authorities (personal communication).
Another is under-reporting, which is a serious issue in Brazil. 
...
are not easy to fully determine.
...
of susceptibility to the circulating virus(es).
To estimate the susceptibles we have set the 
transmissibility from the $R_t$ series derived from data~\cite{nishiura} 
(\ref{eq:effbeta}). 
Then, 
...
dengue transmission based on a single-strain Susceptible-Infectious-Removed 
(SIR) model.
The reason for the choice of a single strain model, is the lack of 
 information regarding the actual proportions of each virus in 
circulation.
...
based only on clinical criteria. Laboratory confirmation and serotype 
...
procedures. The normalization was done by dividing the number of cases reported 
...
indistinguishable from 1. Due to the intrinsic variability of the \rr series, 
the examination of its credible intervals is essential to identify periods 
of sustained transmission. The wider intervals between epidemics are due to the 
scarcity of cases during these periods. Our methodology to derive the 
probability distribution of \rr is an original contribution applicable to the 
...
larger epidemics, in terms of peak size are not the one with the 
greater attack ratios. This stresses the importance of knowing the 
immunological structure of the population in order to truly assess the impact 
of any epidemic. 
